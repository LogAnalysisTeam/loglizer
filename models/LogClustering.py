...
class LogClustering(object):
    ...
    def _offline_clustering(self, X):
        logger.info('Starting offline clustering...')
        ...
        logger.info('Processed {} instances.'.format(X.shape[0]))
        logger.info('Found {} clusters offline.\n'.format(len(self.representatives)))
        # logger.info('The representive vectors are:')
    ...
    def _online_clustering(self, X):
        logger.info("Starting online clustering...")
        for i in range(self.num_bootstrap_samples, X.shape[0]):
            if (i + 1) % 2000 == 0:
                logger.info('Processed {} instances.'.format(i + 1))
            instance_vec = X[i, :]
        ...
        logger.info('Processed {} instances.'.format(X.shape[0]))
        logger.info('Found {} clusters online.\n'.format(len(self.representatives)))
        # logger.info('The representive vectors are:')
        # pprint.pprint(self.representatives.tolist())
